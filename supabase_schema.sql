...
-- =============================
-- EmbedX 2.0 Registration Table
-- =============================
CREATE TABLE IF NOT EXISTS embedx2_registrations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  team_name TEXT NOT NULL,
  leader_name TEXT NOT NULL,
  leader_email TEXT NOT NULL,
...
ALTER TABLE IF EXISTS embedx2_registrations ENABLE ROW LEVEL SECURITY;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_policy WHERE polname = 'allow_public_insert_embedx2_registrations') THEN
    EXECUTE 'CREATE POLICY allow_public_insert_embedx2_registrations ON embedx2_registrations FOR INSERT WITH CHECK (true)';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_policy WHERE polname = 'allow_public_read_embedx2_registrations') THEN
    EXECUTE 'CREATE POLICY allow_public_read_embedx2_registrations ON embedx2_registrations FOR SELECT USING (true)';
  END IF;
END $$;

-- =============================
-- Storage Bucket for EmbedX 2.0 Payment Screenshots
-- =============================
-- NOTE: Create the storage bucket 'embedx2' manually in Supabase Storage UI or via API.